...
function App() {
  ...
  const uploadImage = async (imgData) => {
    ...
    console.log(data);
    const replayData = JSON.parse(data.choices[0].message.content);
    setWord(replayData.representative_word);
    setSentence(replayData.example_sentence);
    setExplainations(replayData.explaination_replys);

    const audioUrl = await generateAudio(replayData.example_sentence);
  }
  return (
