...
- 使用发布者订阅模式思想
    这个实现使用了发布者-订阅者模式的思想，通过 Zustand 状态管理来实现组件间的通信。当喜欢状态发生变化时，相关组件会收到通知并更新 UI。react-vant 的 Tabbar.Item 组件内置了徽章功能，我们只需要设置 badge 属性即可显示微标提示。- mitt - - eventBus 事件总线    
        - 实例化 mitt()
